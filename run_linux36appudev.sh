...
run_bot() {
	...
	if hash python3.6 2>/dev/null; then # TODO abstracify all this which mirrors above an also look up boolean operators in sh
		if hash pip3.6 2>/dev/null; then
			echo "Using global pip3.6 executable"
			if pip3.6 install --user -r requirements.txt; then
				echo "Starting bot..."
				python3.6 loopself.py
				ret=$?
				if [ $ret == "15" ]; then
					min_updater
					run_bot
			...
			echo "Using pip3.6 as a python3.6 module"
			echo "Upgrading pip"
			if python3.6 -m pip3.6 install --user --upgrade pip3.6; then
				echo "Upgrading requirements"
				if python3.6 -m pip3.6 install --user --upgrade -r requirements.txt; then
					echo "Starting bot..."
					python3.6 loopself.py
					ret=$?
					if [ $ret == "15" ]; then
				...
				echo "pip3.6 could not be installed. Try using your package manager"
				exit 253
			fi
		fi
		...
		if hash pip3.6 2>/dev/null; then
			...
			echo "Using pip3.6 as a python3.6 module"
			echo "Upgrading pip"
			if python -m pip3.6 install --user --upgrade pip3.6; then
				echo "Upgrading requirements"
				if python -m pip3.6 install --user -r requirements.txt; then
					echo "Starting bot..."
					python loopself.py
					ret=$?
					if [ $ret == "15" ]; then
